...
from custom_training.custom_cosypose.datasets.datasets_cfg import make_scene_dataset
...
from cosypose.training.maskrcnn_forward_loss import h_maskrcnn
from cosypose.training.detector_models_cfg import create_model_detector, check_update_config
from cosypose.training.train_detector import collate_fn, run_eval, log
...
def train_detector(args):
    ...
    def make_datasets(dataset_names, custom_ds_dir):
        ...
        for (ds_name, n_repeat) in dataset_names:
            ...
            all_labels = all_labels.union(set(ds.all_labels_short))
            for _ in range(n_repeat):
                datasets.append(ds)
        return ConcatDataset(datasets), all_labels

    scene_ds_train, train_labels = make_datasets(
    ...
    # Following lines are identical to original CosyPose's file
    ds_kwargs = dict(
    ...
    # Same training as original CosyPose
    if args.resume_run_id:
        resume_dir = EXP_DIR / args.resume_run_id
    ...
    if args.run_id_pretrain is not None:
        ...
        def keep(k): return 'box_predictor' not in k and 'mask_predictor' not in k
