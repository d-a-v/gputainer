...
# Pose models
from custom_training.custom_cosypose.models.pose import CustomPosePredictor
...
def create_model_pose(cfg, renderer, mesh_db):
    ...
    if backbone_str == 'efficientnet-b3':
        backbone = EfficientNet.from_name(
            'efficientnet-b3', in_channels=n_inputs)
    ...
    render_size = (240, 320)
    model = CustomPosePredictor(backbone=backbone,
                                renderer=renderer,
                                mesh_db=mesh_db,
