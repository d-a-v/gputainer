...
from custom_training.custom_cosypose.rendering.bullet_scene_renderer import BulletSceneRenderer


def init_renderer(urdf_ds, preload=True, urdf_dataset_dir=None):
    renderer = BulletSceneRenderer(urdf_ds=urdf_ds,
                                   preload_cache=preload,
                                   background_color=(0, 0, 0),
                                   urdf_dataset_dir=urdf_dataset_dir)
    return renderer


def worker_loop(worker_id, in_queue, out_queue, object_set, preload=True, urdf_dataset_dir=None):
    ...
    while True:
        ...
        if is_valid:
            ...
            depth = np.stack([d['depth'] for d in cam_obs]
...
class BulletBatchRenderer:
    def __init__(self, object_set, n_workers=8, preload_cache=True, urdf_dataset_dir=None):
        self.object_set = object_set
        self.n_workers = n_workers
        self.urdf_dataset_dir = urdf_dataset_dir
    ...
    def render(self, obj_infos, TCO, K, resolution=(240, 320), render_depth=False):
        ...
        for n in np.arange(bsz):
            ...
            kwargs = dict(cam_infos=[cam_info], obj_infos=[
                          obj_info], render_depth=render_depth)
            if self.n_workers > 0:
                kwargs['data_id'] = n
        ...
        images = torch.as_tensor(
            np.stack(images, axis=0)).pin_memory().cuda(non_blocking=True)
        images = images.float().permute(0, 3, 1, 2) / 255

        if render_depth:
            depths = torch.as_tensor(
                np.stack(depths, axis=0)).pin_memory().cuda(non_blocking=True)
            depths = depths.float()
            ...
            for n in range(self.n_workers):
                plotter = multiprocessing.Process(target=worker_loop,
                                                  kwargs=dict(worker_id=n,
                                                              ...
                                                              urdf_dataset_dir=self.urdf_dataset_dir))
                plotter.start()
                self.plotters.append(plotter)
        else:
            self.plotters = [init_renderer(
                self.object_set, preload_cache, urdf_dataset_dir=self.urdf_dataset_dir)]
