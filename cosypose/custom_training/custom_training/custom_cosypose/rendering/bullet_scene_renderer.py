...
from custom_training.custom_cosypose.datasets.datasets_cfg import make_urdf_dataset
...
class BulletSceneRenderer(BaseScene):
    def __init__(self,
                 ...
                 gpu_renderer=True,
                 gui=False,
                 urdf_dataset_dir=None):

        self.urdf_ds = make_urdf_dataset(urdf_ds, urdf_dataset_dir)
        ...
        for (obj_info, body) in zip(obj_infos, bodies):
            ...
            if color is not None:
                pb.changeVisualShape(
                    body.body_id, -1, physicsClientId=0, rgbaColor=color)
        return bodies
    ...
    def render_images(self, cam_infos, render_depth=False):
        ...
        for cam_info in cam_infos:
            ...
            if self.background_color is not None:
                im = cam_obs_['rgb']
                mask = cam_obs_['mask']
                im[np.logical_or(mask < 0, mask == 255)
