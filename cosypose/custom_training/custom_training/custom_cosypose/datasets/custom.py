...
class CustomDataset:
    def __init__(self, ds_dir, split='train', load_depth=False):
        ...
        models_infos = json.loads((ds_dir / 'categories.json').read_text())
        self.all_labels = list(models_infos.keys())
        self.all_labels_short = [label.split(
            '/')[-1].split('.')[0] for label in self.all_labels]
    ...
    def __getitem__(self, frame_id):
        ...
        if 'scene_gt_info' in self.annotations[scene_id_str]:
            ...
            for n in range(n_objects):
                ...
                name = self.all_labels_short[int(obj_id)]
